...
fn vertexMain(@location(0) pos: vec2f, @builtin(instance_index) idx: u32) -> @builtin(position) vec4f {
    ...
    let halfLength = 1.f; // half grid length
    let gridLength = halfLength * 2.f; // full grid length
    let cell = pos / 10; // divide the input quad into 10x10 pieces
    let offset = - halfLength + uv * gridLength + gridLength / 10 * 0.5; // compute the offset for the instance
    // Apply motor
