...
export default class Camera {
  ...
  moveX(d) {
    ...
    let newpose = PGA3D.applyMotorToPoint(PGA3D.geometricProduct(dt,this._pose));
  ...
  rotateX(d) {
    ...
    // Suggest to use PGA3D
    let dr = PGA3D.createRotor(Math.PI * d, 1, 0, 0); 
    let dt = PGA3D.createTranslator(1, 0, 0);
    let newpose = PGA3D.applyMotorToDir(this._pose,PGA3D.geometricProduct(dt, dr));
    this.updatePose(newpose);
  }
  ...
  rotateY(d) {
    ...
    // Suggest to use PGA3D
    let dr = PGA3D.createRotor(Math.PI * d, 0, 1, 0); 
    let dt = PGA3D.createTranslator(0, 1, 0);
    let newpose = PGA3D.applyMotorToDir(this._pose,PGA3D.geometricProduct(dt, dr));
    this.updatePose(newpose);
  }
  ...
  rotateZ(d) {
    ...
    // Suggest to use PGA3D
    let dr = PGA3D.createRotor(Math.PI * d, 0, 0, 1); 
