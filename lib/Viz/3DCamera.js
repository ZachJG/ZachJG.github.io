...
export default class Camera {
  ...
  rotateX(d) {
    ...
    // Suggest to use PGA3D
    let dr = PGA3D.createRotor(Math.PI, d, 0, 0); 
    let dt = PGA3D.createTranslator(d, 0, 0);
    let newpose = PGA3D.applyMotorToPoint(this._pose,PGA3D.geometricProduct(dt, dr));
    this.updatePose(newpose);
  }
  ...
  rotateY(d) {
    ...
    // Suggest to use PGA3D
    let dr = PGA3D.createRotor(Math.PI, 0, d, 0); 
    let dt = PGA3D.createTranslator(0, d, 0);
    let newpose = PGA3D.applyMotorToPoint(this._pose,PGA3D.geometricProduct(dt, dr));
    this.updatePose(newpose);
  }
  ...
  rotateZ(d) {
    ...
    // Suggest to use PGA3D
    let dr = PGA3D.createRotor(Math.PI, 0, 0, d); 
    let dt = PGA3D.createTranslator(0, 0, d);
    let newpose = PGA3D.applyMotorToPoint(this._pose,PGA3D.geometricProduct(dt, dr));
    this.updatePose(newpose);
  }
